...
class BPETokenizer:
    ...
    def _build_vocab(self):
        ...
        common_subwords = [
            ...
            'able', 'ful', 'less', 'the', 'and', 'ing ', 'ed ', 'tion ',
            'question', 'answer', 'context', 'what', 'when', 'where', 'who',
    ...
    def _get_word_tokens(self, word: str) -> List[str]:
        ...
        while i < len(word):
            matched = False
            for length in range(min(15, len(word) - i), 0, -1):
    ...
    def encode(self, text: str, max_length: int = 192, add_eos: bool = True) -> List[int]:
        """编码文本"""
...
class C4Dataset(Dataset):
    def __init__(self, data_path: str, tokenizer, max_length: int = 192, noise_density: float = 0.15):
...
class SQuADDataset(Dataset):
    def __init__(self, data_path: str, tokenizer, max_length: int = 192, target_max_length: int = 48):
        self.tokenizer = tokenizer
        self.max_length = max_length
...
def pretrain(model, dataloader, optimizer, device, num_steps=300):
    model.train()
    total_loss = 0.0
    step = 0
...
def main():
    print("=" * 80)
    print("T5 (Text-to-Text Transfer Transformer) 轻量级实现")
    print("=" * 80)
    ...
    c4_dataloader = DataLoader(c4_dataset, batch_size=16, shuffle=True, num_workers=0)
    
    print(f"\n开始预训练 (300 steps)...")
    pretrain_start = time.time()
    pretrain_loss = pretrain(model, c4_dataloader, optimizer, device, num_steps=300)
    ...
    train_subset_size = min(5000, len(squad_train_dataset))
    ...
    squad_train_loader = DataLoader(squad_train_subset, batch_size=16, shuffle=True, num_workers=0)
    squad_val_loader = DataLoader(squad_val_subset, batch_size=16, shuffle=False, num_workers=0)
