...
class Config:
    ...
    pretrain_steps: int = 800   # 快速预训练
    finetune_steps: int = 250   # 快速微调
...
class BPETokenizerWrapper:
    ...
    def train(self, corpus_files: List[str]):
        """在语料上训练BPE分词器"""
        trainer = BpeTrainer(
        ...
        print(f"BPE词汇表大小: {self.tokenizer.get_vocab_size()}")
...
class WikiTextDataset(Dataset):
    """WikiText 预训练数据集"""
    def __init__(self, data_path: str, tokenizer: BPETokenizerWrapper, block_size: int, 
                 is_train: bool = True, max_samples: int = 150):
        ...
        print(f"加载了 {len(tokens)} 个 BPE tokens")
    ...
    def __init__(self, data_path: str, tokenizer: BPETokenizerWrapper, block_size: int,
        ...
        try:
            ...
            for i in range(max_samples):
                text = f"This is a sample review number {i}. It contains some words to test the model performance."
                label = i % 2
                tokens = tokenizer.encode(text, max_length=block_size)
                self.examples.append((tokens, label))
...
def train_finetune(model: GPTForClassification, train_loader: DataLoader, config: Config):
    """微调"""
...
# 评测函数
# =============================================================================
def evaluate_generation(model: GPT, tokenizer: BPETokenizerWrapper, config: Config):
    """评测文本生成"""
    print("\n" + "="*80)
    print("文本生成评测")
    print("="*80)
    ...
    with torch.no_grad():
        for prompt in prompts:
            ...
            generated = model.generate(x, max_new_tokens=50, temperature=0.8)
...
def main():
    ...
    tokenizer = BPETokenizerWrapper(vocab_size=config.vocab_size)
    
    # 准备BPE训练数据
    corpus_files = []
    for fname in ['wiki.train.tokens', 'wiki.valid.tokens']:
        fpath = os.path.join(pretrain_data_path, fname)
        if os.path.exists(fpath):
            corpus_files.append(fpath)
    
    if corpus_files:
        print(f"在以下文件上训练BPE: {corpus_files}")
        tokenizer.train(corpus_files)
        config.vocab_size = tokenizer.get_vocab_size()
        print(f"BPE分词器训练完成! 词汇表大小: {config.vocab_size}")
    else:
        print("警告: 未找到训练数据文件!")
        return
    ...
    train_pretrain(model, pretrain_loader, config)
    ...
    train_finetune(classifier_model, imdb_loader, config)
    ...
    print("训练完成总结")
    print("="*80)
    print(f"分词器: BPE (Byte Pair Encoding)")
