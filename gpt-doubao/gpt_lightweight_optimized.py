...
class CharTokenizer:
    ...
    def _build_vocab(self):
        ...
        self.id_to_token = {idx: token for idx, token in enumerate(all_tokens)}
    
    def tokenize(self, text: str) -> List[int]:
        """将文本转换为字符级 token id"""
        return [self.token_to_id.get(c, self.token_to_id['<UNK>']) for c in text]
...
# =============================================================================
# 3. 数据集与数据加载器 (优化版 - 更少数据)
# =============================================================================
class WikiTextDataset(Dataset):
    """WikiText 预训练数据集 (优化版)"""
    def __init__(self, data_path: str, tokenizer, block_size: int = 64, max_lines: int = 3000):
        self.block_size = block_size
        self.tokenizer = tokenizer
        
        # 读取数据（只取前 max_lines 行以加速）
        with open(data_path, 'r', encoding='utf-8') as f:
            ...
            for i, line in enumerate(f):
                if i >= max_lines:
                    break
                line = line.strip()
                if line and not line.startswith('='):  # 跳过标题行和空行
                    lines.append(line)
...
class IMDBDataset(Dataset):
    """IMDB 情感分类数据集 (优化版)"""
    def __init__(self, data_df, tokenizer, block_size: int = 64):
        self.tokenizer = tokenizer
        self.block_size = block_size
        self.data = []
        self.labels = []
...
def train_pretrain(
    ...
    """预训练阶段：Next Token Prediction (优化版)"""
...
# 5. 评测函数
# =============================================================================
def evaluate_generation(model: GPT, tokenizer, device: torch.device, prompts: List[str]):
    """评测文本生成能力"""
    model.eval()
    print("\n" + "="*80)
    print("文本生成评测")
...
def main():
    ...
    valid_path = r"E:\Program\python\dogfooding\pre-trained models\datasets\WikiText\wiki.valid.tokens"
    
    train_dataset = WikiTextDataset(
        pretrain_path,
        tokenizer,
        block_size=config['block_size'],
        max_lines=2000  # 减少数据量加速
    )
    val_dataset = WikiTextDataset(
        valid_path,
        tokenizer,
        block_size=config['block_size'],
        max_lines=500  # 减少数据量加速
    )
    ...
    # 分割验证集
    val_df = train_df.sample(n=400, random_state=42).reset_index(drop=True)
